...
if TYPE_CHECKING:
    from schema import DocumentStructureBlueprint
...
def build_ast_from_blueprint(
    *,
    ...
    blueprint_content: Optional[str] = None,
    text_content: Optional[str] = None,
    blueprint: Optional["DocumentStructureBlueprint"] = None,
    ...
    引数:
        blueprint_path: blueprint JSON ファイルのパス（blueprint_content, blueprint が指定されている場合は不要）。
        text_path: テキストファイルのパス（text_content が指定されている場合は不要）。
        blueprint_content: blueprint JSON の内容（文字列）。仮想ファイルシステムからの読み込み用。
        text_content: テキストの内容（文字列）。仮想ファイルシステムからの読み込み用。
        blueprint: DocumentStructureBlueprint オブジェクト。直接渡す場合に使用。
    ...
    """
    # blueprint の読み込み（オブジェクト > content > path の優先順位）
    blueprint_dict: Dict[str, Any]
    if blueprint is not None:
        # DocumentStructureBlueprint オブジェクトを辞書に変換
        blueprint_dict = blueprint.model_dump()
    elif blueprint_content is not None:
        blueprint_dict = json.loads(blueprint_content)
    elif blueprint_path is not None:
        blueprint_dict = _load_json(blueprint_path)
    else:
        raise ValueError("blueprint, blueprint_path, blueprint_content のいずれかを指定してください")
    ...
    meta_rules, structural_rules = _compile_rules(blueprint_dict)
    exclusion_patterns = _compile_exclusions(blueprint_dict)
