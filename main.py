...
import src.blueprint_tools as blueprint_tools
...
from src.tools import extract_regex_matches,read_text_segment,get_file_length,read_text_file, compare_setup, compare_all_chunk_similarity_matching, compare_get_grouping, compare_search_by_keyphrase, compare_get_chunk, compare_specified_chunks_diff, compare_specified_chunks_llm, read_ast, COMPARE_STATE
...
from src.blueprint_tools import preview_blueprint_headings, validate_blueprint
...
# %% #blueprint作成
## ツールの定義
tools_blueprint_builder = [read_text_segment, read_text_file, extract_regex_matches, get_file_length] 
tools_blueprint_validator = [read_text_segment, read_text_file, extract_regex_matches, get_file_length, preview_blueprint_headings, validate_blueprint] 
...
## ファイル内容を読み込む
with open(os.path.join("data", "input", target_file), "r", encoding="utf-8") as f:
    target = f.read()

## 仮想ファイルシステムを設定
initial_files_blueprint_builder = {
    f"/{target_file}": create_file_data(target)
}
## promptの設定
...
# %% #AST構築
## AST構築
ast_path = os.path.join("data", "ast", f"{target_file}.ast.json") 
ast = blueprint_ast_builder.build_ast_from_blueprint(
    blueprint=blueprint,
    text_path=os.path.join("data", "input", target_file),
...
# %% #比較種別判定の準備
## ツールの定義
tools_compare_type_analysis = [read_ast, compare_get_grouping, compare_search_by_keyphrase, compare_get_chunk, compare_specified_chunks_diff, compare_specified_chunks_llm]
...
# %% #比較分析の準備
## ツールの定義
tools_compare_analysis = [read_ast, compare_get_grouping, compare_search_by_keyphrase, compare_get_chunk, compare_specified_chunks_diff, compare_specified_chunks_llm]
